---
title: "NHANES Codebook Filtering"
...
Purpose of this document is to obtain descriptions on the variables that is collected for NHANES and to be able to create a .csv file of variables of interest so we can use it to `rnhanesdata` package. 

For my project in particular, we are interested in variables that might be associated with cardiovascular indicators or diseases. While we won't select all of the possible variables, we will try to select those that are "most commonly" known to affect or be related to cardiovascular physiology. 
...
survey_filedata <- 
  ...
  mutate(
    group_filename = gsub(" Doc", "", group_filename)    # remove the "Doc" text at the end of every filename in filedata
    ) %>% 
  ...
  select(-var_url)                                       # remove unneeded column. 
...
Note however that `nrow =` `r nrow(survey_filedata)` has 3 more variables when compared to `distinct(survey_filedata, group_filename) =` `r nrow(distinct(survey_filedata, group_filename))`, suggesting that there are some duplicates (see below).

```{r surveydf_dupe_check}

janitor::get_dupes(survey_filedata, group_filename)
...
Unfortunately, this is one of the more time-consuming portion and tedious as we need to evaluate each variable by eye to determine which variables are of specific interest. Thankfully, our `group_var` variable can provide some insight on which group variables we should look into. List of removed group variables below, based on "irrelevance" to cardiovascular issues:
...
selected_filedata <- survey_filedata %>% 
...
selected_filedata <- left_join(selected_filedata, 
                               detailed_var_desc,
...
janitor::get_dupes(selected_filedata, group_filename, var_name)

```

As we see above, there were `r nrow(janitor::get_dupes(selected_filedata, group_filename, var_name))` rows of duplicates. Having cross-checked with CDC's descriptions that doesn't appear to show any particular difference, we could safely use `distinct()` to filter these out. However, to ensure there's no "missing information", we decided to remove duplicates with exact matches in three columns, `group_filename`, `var_name`, and `det_var_desc`. 

```{r detailedselecteddf_remove_dupe}

selected_filedata <- distinct(selected_filedata, 
...
lim_survey <- det_selected_df %>% 
  filter(survey_type == "limited data") %>% 
  select(-survey_type, -filedata)
...
final_covar_data <- select_if(covar_data_d, names(covar_data_d) %in% final_df$var_name)
