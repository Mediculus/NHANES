...
We now proceed to removing variables which causes issue to our preliminary model. These are typically variable-pairs with large amounts of `NA` that comprised of > 80% of our participants. 
...
Now that we've minimized our covariates further, we can then begin selecting which ones are `factor` vs. `numeric`. We are especially interested in exploring the other factor covariates (besides education) and evaluating their distribution before committing to our models.

## Converting Data Types and Renaming Variables
...
new_merged_df <- new_merged_df %>% 
  mutate_at(
    .vars = vars("riagendr", "ridreth1", "peascst1", "bpxpuls", "diq010", "diq190a", "diq190b", 
...
revised_merged_df %>% count(race_eth)
...
This variable is an indicator for the blood pressure measurement. Coded as follows:
...
revised_merged_df %>% count(bp_status)

revised_merged_df %>% 
  group_by(bp_status) %>% 
  count(diabetes) %>% 
  ggplot(aes(x = diabetes, y = n, color = bp_status, fill = bp_status)) + 
  geom_bar(stat = "identity", position = "dodge")

```

As seen above, it seems that we have several datapoints which only has partial BP measurement as well as "not done". Nevertheless, we are unlikely to remove these from our dataset because this could reflect on the "real-world" occurrences and thus might be valuable. 

#### `bpxpuls`: quality of pulse  

An indicator for the kind of pulse for our heart; it is either:
...
* `diq010`: Told by doctor to have diabetes
  ...
  * 7 = refused
  * 9 = don't know
* `diq160`: Told by doctor to have pre-diabetes (note that from `diq160` - `pfq020`, they all have the same value code)
  * 1 = Yes
  * 2 = No
  * 7 = Refused
  * 9 = Don't know
...
First, we'll evaluate the amount of "don't know", "refused" or similar in each of these categorical variables. For most categorical variables, we can achieve this using the `skim()` function from `skimr` package since most of the levels are around 3. Two exceptions are `ridreth1` and `indfminc` variables. 

```{r}

skimr::skim_without_charts(new_merged_df)

new_merged_df %>% count(ridreth1)
new_merged_df %>% count(indfminc)

```


We then convert these variables into the corresponding data type in our dataset. 


# Evaluating associations

To see whether or not pursuing some sort of predictive modeling is even worth our endeavor, we would use a simple GLM to model a saturated model based on our remaining covariates to estimate diabetic status (i.e. a logistic model). We removed `diq010` and `seqn` because `diq010` is our outcome variable while `seqn` is participant ID, which doesn't make sense to include in the model. 

```{r, results = "markup"}

saturated_model <- glm(diabetes ~ . -diabetes -pid, family = binomial, data = revised_merged_df, na.action = na.omit)

summary(saturated_model)

names(revised_merged_df)
...
#revised_merged_df %>% write_csv("./Datasets/cleaned_dataset.csv")
