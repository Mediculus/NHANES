...
We first merge our covariate dataset with the total activity count dataset. 
...
## Removing Now-Irrelevant Variables

Since we've isolated our dataset, now there are several variables that became irrelevant. We should remove this to reduce our datasets.
...
Now that we've minimized our covariates further, we can then begin selecting which ones are `factor` vs. `numeric`. These are the findings:

* `seqn`: `factor`; identifier for survey respondents
...
* `peascst1`: BP measure status.  
  ...
  2 = partial  
  3 = not done  
* `bpxpls`: value of pulse for ages 8+
