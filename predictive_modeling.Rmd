...
load(file = "../NHANES/_data/nhanes_data.rda")
```
...
model_df <- read_rds(file = "../NHANES/_data/finished_model_dataframe_git.rds")
```
...
lasso_summary <- model_df_long %>% 
  ...
  select(algorithm, resamp_method, metrics_summary, predictions)
...
results %>% left_join(raw_metrics, by = c("algorithm", "resamp_method", ".config")) %>% 
  filter(.metric == "g_mean") %>% 
  select(algorithm, resamp_method, id, .estimate, .metric) %>% 
  filter((resamp_method == "adasyn" & algorithm == "lasso") | 
           (resamp_method == "down" & algorithm == "rf")) %>% 
  # calculate mann whitney U test. function defaults to exact test if n < 50
  wilcox.test(formula = .estimate ~ algorithm, data = .)
```
...
final_dataframe <- read_rds(file = "../NHANES/_data/final_dataframe_git.rds")
```
